...
unsigned long long WorkCtx::doWork(void* firstSrc, void* firstDst, unsigned nThreads) {
    ...
    {
        if (contentSize != 0 && contentSize != readSize) EXM_THROW(38, "End of frame error : %s", LZ4F_getErrorName(LZ4F_errorCode_t(0) - LZ4F_ERROR_frameSize_wrong));
        ...
        if (result < writeSize) EXM_THROW(39, "Write error : cannot write end of frame");
        compressedFileSize += writeSize;
    }
...
unsigned long long LZ4IO_compressFilename_extRess_multithread(const void* pRess, FILE* srcFile, const char* srcFileName, FILE* dstFile,
                                                              const LZ4F_preferences_t* pPrefs, unsigned long long* pCompressedfilesize)
{
    ...
    LZ4IOMT::WorkCtx workers(ioInfo, (const cRess_t*)pRess, pPrefs, parallel);
